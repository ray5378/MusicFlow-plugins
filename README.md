...
| 插件 | 版本 | 类型 | 说明 |
| --- | --- | --- | --- |
| [`go-music-dl`](plugins/go-music-dl) | 1.2.6 | source | 在线源三合一（搜索匹配 / 歌词 / 封面）：对接自建 go-music-dl 服务，为歌单未匹配曲目提供在线补全、播放时单曲匹配，并为本地库补歌词/封面。配置页填**后台用户名/密码**后，插件自动登录并按既有「歌单同步」路径把各平台**我的私人歌单**（网易云/QQ/酷狗/汽水的我喜欢的/收藏）作为本地歌单同步（每日调度 + 手动「同步所有平台」触发）。不部署该服务则无需安装 |
