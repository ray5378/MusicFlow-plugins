...
| [`listenbrainz`](plugins/listenbrainz) | 1.5.2 | scrobbler | 播放记录上报 + 推荐歌单：把播放事件上报到 [ListenBrainz](https://listenbrainz.org)（开源的 Last.fm 替代品），并按协同过滤推荐生成「ListenBrainz」歌单（可固定首页、每日调度 + 手动刷新，本地/在线源补全）。v1.5.2 修复：MusicBrainz 兜底改为带预算 + 失败快速放弃，MB 不可达时自动降级为仅 LB 元数据、不再 15s 超时报错。不用这类服务的话无需安装 |
