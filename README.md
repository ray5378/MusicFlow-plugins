...
| [`listenbrainz`](plugins/listenbrainz) | 1.5.4 | scrobbler | 播放记录上报 + 推荐歌单：把播放事件上报到 [ListenBrainz](https://listenbrainz.org)（开源的 Last.fm 替代品），并按协同过滤推荐生成「ListenBrainz」歌单（可固定首页、每日调度 + 手动刷新，本地/在线源补全）。v1.5.4：在线补全加 **10s 预算闸**——超预算的歌留外部占位、由后台 auto-match 补全（主进程不受沙箱 15s 限制），彻底修复「推荐歌单生成超时失败」；ListenBrainz API 超时 20s→10s。v1.5.3 起：换名优先用「收听历史 + LB 元数据」（直连可达、不依赖 MB），MusicBrainz 仅兜底且带重试/预算/快速降级；配置页新增「走系统网络代理」开关（需后端 1.7.38+）与「排除已听过的推荐」开关。不用这类服务的话无需安装 |
